...
class CartController extends AbstractController
{
    ...
    public function validateBasket(
        ...
        $newOrderProduct = new Order();
        /** @var User $user */
        $user = $this->getUser();
            foreach ($newCart as $products) {
                foreach ($products as $product) {
            $newOrderProduct->addProduct($product);
            $newOrderProduct->setPrice($product->getPrice());
            $newOrderProduct->getId($newOrderProduct);
            $newOrderProduct->setClient($user);
            $ema->persist($newOrderProduct);
        }
    }
    $newOrderProduct->setPrice($data['total']);
        $ema->flush();
        $session->set("cart", []);
